...
// import { deleteContact } from '../api/users'; // deletes from contacts
...
//         const others = all.flatMap(c => c.participants).filter(p => p.shareId !== user.shareId);
//         const unique = Array.from(new Map(others.map(p => [p.shareId, p])).values());
//         setContacts(unique);
//       })
//       .catch(console.error);
//   }, [user]);
...
//       setContacts(prev => prev.filter(c => c.shareId !== deleteTarget));
//     } catch {
...
//     if (selectionMode) { toggleSelect(shareId); return; }
//     try {
...
//                 onLongPressStart={id => {
//                   longPressTimers.current[id] = window.setTimeout(() => { setSelectionMode(true); setSelected(new Set([id])); }, 600);
//                 }}
...
//         setSwiping(true);
//         setTimeout(() => { setSwiping(false); onSwipeComplete(); }, 300);
//       }
//     },
...
//       className="relative overflow-hidden flex items-center justify-between px-5 py-4 md:px-3 md:py-2 bg-white rounded-xl shadow-md hover:bg-gray-100 cursor-pointer"
      
//     >
//       <div className={`absolute inset-0 bg-red-500 transition-opacity duration-300 ${swiping ? 'opacity-100' : 'opacity-0'}`} />
//       <div className={
//         `flex items-center space-x-4 md:space-x-3 flex-1 transition-transform duration-300 ease-in-out ${swiping ? 'translate-x-full' : 'translate-x-0'}`
//       }>
//         <div className="h-12 md:h-8 w-12 md:w-8 rounded-full flex-shrink-0" style={{ backgroundColor: `#${contact.shareId.slice(-6)}` }} />
//         <div className="flex-1">
//           <div className="text-lg md:text-base font-semibold text-gray-900 truncate" title={contact.name}>{contact.name}</div>
//           <div className="text-sm text-gray-500">{selectionMode ? (isSelected ? 'Selected' : 'Tap to select') : 'Tap to message'}</div>
//         </div>
//       </div>
...
//   const openChat = (shareId: string) => {
//     if (selectionMode) {
//       toggleSelect(shareId);
//       return;
//     }
//     navigate(`/chat/${shareId}`);
//   };
...
//         {/* content */}
//         <div className="flex-1 px-5 py-4 md:px-3 md:py-2 overflow-y-auto">
//           <h1 className="text-3xl md:text-2xl font-bold mb-6 text-gray-800">Your Contacts</h1>
...
//               onChange={e => setQuery(e.target.value)}
//               placeholder="Search contacts…"
//               className="w-full pl-12 pr-4 py-3 bg-gray-100 rounded-full focus:outline-none focus:ring-2 focus:ring-cyan-300 text-base"
//             />
//           </div>
...
//   contact, isSelected, selectionMode,
//   onLongPressStart, onLongPressEnd, onSelect, onOpen, onSwipeComplete
...
//       onTouchEnd={() => isDesktop && onLongPressEnd(contact.shareId)}
//       onClick={() => (selectionMode ? onSelect() : onOpen())}
//       className="relative overflow-hidden flex items-center justify-between px-5 py-4 bg-white rounded-xl shadow-md hover:bg-gray-100 cursor-pointer"
//     >
//       <div className={`absolute inset-0 bg-red-500 transition-opacity duration-300 ${swiping ? 'opacity-100' : 'opacity-0'}`} />
